 ...
 */
 ...
 const Agent* Behavior::determineActiveAgent(Agent& self, const std::vector<const Agent*>& otherAgents, bool assign) const
 {
   ...
   // Fewer field players usually means larger spatial separation and bigger discrepancies between local ball models.
   // Therefore, the decision margin is increased a bit in small-team situations.
   const std::size_t numOfFieldPlayers = (self.isGoalkeeper ? 0u : 1u) +
                                         static_cast<std::size_t>(std::count_if(otherAgents.begin(), otherAgents.end(), [](const Agent* agent)
   {
     return !agent->isGoalkeeper;
   }));
   const float activeDecisionMargin = numOfFieldPlayers <= 2 ? 0.2f :
                                      numOfFieldPlayers == 3 ? 0.15f :
                                      0.1f;
 
   // Compare two candidates for the active role. If one candidate is clearly better, it wins.
   // If both are close, prefer the previously active agent. If there is still no difference,
   // use the player number as a stable tie breaker so that all robots can reach the same decision.
   const auto isBetterActiveCandidate = [&](const Agent& candidate,
                                            float candidateTTRB,
                                            const Agent& currentBest,
                                            float currentBestTTRB) -> bool
   {
     const float diff = candidateTTRB - currentBestTTRB;
     if(diff < -activeDecisionMargin)
       return true;
     if(diff > activeDecisionMargin)
       return false;
 
     const bool candidateWasTheOnlyActive = candidate.number == theOneAndOnlyPreviouslyActiveAgent;
     const bool currentBestWasTheOnlyActive = currentBest.number == theOneAndOnlyPreviouslyActiveAgent;
     if(candidateWasTheOnlyActive != currentBestWasTheOnlyActive)
       return candidateWasTheOnlyActive;
 
     return candidate.number < currentBest.number;
   };
   ...
   if(canBeActive(self, self.number == theOneAndOnlyPreviouslyActiveAgent, true))
   {
     ...
     for(const Agent* agent : otherAgents)
     {
       ...
       if(isBetterActiveCandidate(*agent, itsTTRB, *minAgent, minTTRB))
       {
         minTTRB = itsTTRB;
         minAgent = agent;
       }
   ...
   else if(assign && canBeActive(self, self.role == ActiveRole::toRole(ActiveRole::closestToTeamBall), false) && theTeamBallModel.isValid)
   {
     ...
     for(const Agent* agent : otherAgents)
     {
       ...
       if(!canBeActive(*agent, itWasActive, true))
       {
         ...
         if(isBetterActiveCandidate(*agent, itsTTRB, *minAgentWithoutBall, minTTRBWithoutBall))
         {
       ...
       if(assign)
         DRAW_TEXT("behavior:activeRole", agent->pose.translation.x(), agent->pose.translation.y(), 100, ColorRGBA::red, std::to_string(static_cast<int>(itsTTRB)));
       if(!minAgentWithBall || isBetterActiveCandidate(*agent, itsTTRB, *minAgentWithBall, minTTRBWithBall))
       {
     ...
     if(minAgentWithoutBall &&
        minTTRBWithoutBall + ttrbWithoutBallPenalty < minTTRBWithBall)
     {
       const_cast<Agent*>(minAgentWithoutBall)->nextRole = ActiveRole::toRole(ActiveRole::closestToTeamBall);
       if(!minAgentWithBall)
         return minAgentWithoutBall;
     }
     if(minAgentWithBall)
   ...
   else
   {
     ...
     for(const Agent* agent : otherAgents)
     {
       ...
       if(assign)
         DRAW_TEXT("behavior:activeRole", agent->pose.translation.x(), agent->pose.translation.y(), 100, ColorRGBA::red, std::to_string(static_cast<int>(itsTTRB)));
       if(!minAgent || isBetterActiveCandidate(*agent, itsTTRB, *minAgent, minTTRB))
       {
         minTTRB = itsTTRB;
         minAgent = agent;
       }
